...
ec2-list () {
	...
	--query 'Reservations[].Instances[].{Id:InstanceId, Name:Tags[?Key==`Name`]|[0].Value, Service:Tags[?Key==`Service`]|[0].Value, _Role:Tags[?Key==`Role`]|[0].Value, Env:Tags[?Key==`Env`]|[0].Value, __PrivateIpAdress:PrivateIpAddress} | sort_by(@, &@.Name)' \
	--output table
}
