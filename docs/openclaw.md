...
>
> **No namespace-based sandboxing since 2026.9.6.** In a user unit, every namespace-based setting — `ProtectSystem`, `ReadWritePaths`, `PrivateTmp`, `ProtectKernelTunables`, `ProtectControlGroups`, `ProtectHostname`… — only applies under `PrivateUsers=true`: an unprivileged user manager cannot create mount namespaces otherwise, and systemd silently skips them (`namespace setup is prohibited … ignoring` in the journal; verified on systemd 252). Until 2026.9.5 the unit set `PrivateUsers=true` for exactly that reason.
>
> 2026.9.6 made that combination unusable. Its fs-safe temp workspaces (used by every claude-cli agent turn and by memory-core narratives) walk the whole ancestry of the temp root from `/` down and require each component to be owned by the effective uid or by uid 0 (`temp-workspace-admission.js` in `@openclaw/fs-safe` 0.18.1, no option to relax it). Inside the user namespace root is unmapped and shows as `nobody` (65534), so `/` always fails: `temp workspace directory has an untrusted owner`, and the agent can no longer answer. Reproduced with `systemd-run --user -M claw@ -p PrivateUsers=yes|no` running `withTempWorkspace()`: only `no` passes. There is no way around it in a user unit: mapping root needs a privileged manager, and the updater only drives user units ([4.4](#44-self-update-on-request)).
>
> So `PrivateUsers` and the settings that depend on it are gone. For an unprivileged uid that was defense in depth only: `/`, `/usr`, `/etc`, `/proc/sys` and the cgroup tree outside its own delegation are not writable by `claw` anyway. What is really lost is the private `/tmp`. OpenClaw's own temp files go to `TMPDIR=~/.cache/openclaw/tmp` and to its hard-coded `/tmp/openclaw`, which now sits in the host's shared `/tmp`, emptied at every boot, where another local account (`debian`, `fastmail-mcp`) could create it first: the gateway would then refuse it (`untrusted owner`) or write into a directory it does not control. The role therefore reserves it with `/etc/tmpfiles.d/openclaw.conf` (`d /tmp/openclaw 0700 claw claw -`), which recreates it for `claw` at every boot, before the user manager starts, and re-asserts owner and mode if it already exists. Restore all of them together if upstream ever accepts the overflow uid.

Settings kept (seccomp- or `no_new_privs`-based, which a user manager applies without namespaces):
...
**Known issue — the 2026.9.5 updater stops the gateway and never restarts it.** On 2026-09-24 a chat-requested 9.5 → 9.6 update staged the package fine, then timed out after 315 s taking its pre-swap read-only snapshot of `state/openclaw.sqlite` while the gateway was still serving (`SQLite read-only snapshot timed out … Stop the Gateway service … then retry`). By then it had already stopped the gateway; it refused the swap, marked the run `serviceRestartSafe: false`, and exited — leaving the service `inactive (dead)` with exit 0, so `Restart=always` did not apply, and the old package intact. Signs: gateway dead, `sudo openclaw-admin update status --json` showing an `activeRun` in phase `activating` whose driver PID no longer exists, and the helper log in `~claw/.cache/openclaw/tmp/openclaw-update-run-handoff-*/handoff.log`. Recovery:

```bash
ssh debian@claw "sudo systemctl --user -M claw@ start openclaw-gateway"  # old release, untouched
ssh debian@claw "sudo openclaw-admin update repair --yes"                # reconciles the abandoned run; its
                                                                          # Doctor step fails while the gateway runs — harmless
ssh debian@claw "sudo systemctl --user -M claw@ stop openclaw-gateway"
ssh debian@claw "sudo openclaw-admin backup create --output /home/claw/backups --verify"
ssh debian@claw "sudo openclaw-admin update --tag <version> --yes"      # with the gateway stopped: snapshot succeeds,
                                                                          # and the updater starts the gateway itself
```
