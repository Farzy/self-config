...
>
> So `PrivateUsers` and the settings that depend on it are gone. For an unprivileged uid that was defense in depth only: `/`, `/usr`, `/etc`, `/proc/sys` and the cgroup tree outside its own delegation are not writable by `claw` anyway. What is really lost is the private `/tmp`; OpenClaw's own temp files go to `TMPDIR=~/.cache/openclaw/tmp` and `/tmp/openclaw` (mode `0700`), so nothing of it lands world-readable. Restore all of them together if upstream ever accepts the overflow uid.
