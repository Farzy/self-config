...
`lipo -archs` over every executable on the login `PATH`: 2084 Mach-O binaries,
121 without an arm64 slice — all in two dead trees, since removed:

- **`/opt/X11`** (114 bins, 178 MB) — orphaned XQuartz; the `.app`, its receipt
  and launchd job were already gone. Also dropped `/etc/paths.d/40-XQuartz` and
  `/etc/manpaths.d/40-XQuartz`. XQuartz has no arm64 build; reinstall under
  Rosetta if X11 is ever needed again.
- **`/usr/local/bin`** (7) — `powerline` (dead `powerline-status` client; shell
  uses powerlevel10k); `wish8.6` `tclsh8.6` `info` `infokey` `install-info`
  (stale 2015/2020 CRAN-R Tcl/texinfo — the arm64 R install keeps native builds
  in `/opt/R/arm64/bin/`); `pdftk` (→ `/opt/pdflabs`, x86 PDFtk Server 2015,
  removed via its uninstaller).

Also swept from `/usr/local` (not on `PATH`, pure x86 cruft): MySQL 5.1.51 for
OS X 10.6 (2010, 343 MB) and a stray 2017 Apple Remote Desktop settings pkg.
`/opt/homebrew/bin` (934 Mach-O), `~/.cargo/bin`, `~/.nvm`, the project venv,
`/usr/local/MacGPG2/bin`, and the system dirs are 100% arm64.

> Cleanups performed by `intel-app-cleanup.sh` and `intel-bin-cleanup.sh` (not
> committed — one-offs kept with the migration scratch). App bundles trash via
> Finder (restorable); the CLI script uses `sudo` for the `/opt` and `/usr/local`
> trees. Both dry-run by default, `--apply` to execute.
